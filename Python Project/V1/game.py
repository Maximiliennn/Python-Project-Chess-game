...
class Game:
    ...
    def start(self):
        """Start the game loop, accepting user input for moves."""
        print("Welcome to Chess! White moves first.")
        self.board.display()

        while True:
            try:
                user_input = input(f"{self.current_player.capitalize()}'s move (e.g., 'e2e4'): ").strip()
                move = self.parse_move(user_input)
                if move is None:
                    print("Invalid input. Please enter a move like 'e2e4' or 'e2 e4'.")
                    continue
